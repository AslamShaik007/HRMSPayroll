...
class Command(BaseCommand):
    """
    Calculate accruel leaves

    AJAY, 01.05.2023
    """

    help = "manage.py calculate_accruel_leaves --commit"
                ...
                for rel in EmployeeLeaveRuleRelation.objects.filter(employee=emp):
                    ...
                    if emp_start_date.year < today.year:
                        if rule.accrual_frequency.lower() == "monthly":
                            accruel_days = 30
                        if rule.accrual_frequency.lower() == "quarterly":
                            accruel_days = 31 * 3
                        if rule.accrual_frequency.lower() == "half_yearly":
                            accruel_days = 31 * 6
                    elif (
                        ...
                        if rule.accrual_frequency.lower() == "quarterly":
                            accruel_days = 31 * 3
                            # accruel_days = (today - emp_start_date).days
                        if rule.accrual_frequency.lower() == "half_yearly":
                            accruel_days = 31 * 6
                            # accruel_days = (today - emp_start_date).days
                    else:
