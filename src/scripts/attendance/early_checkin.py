...
class EmplEarlyCheckinData:
    ...
    def main(self):
        ...
        if Holidays.objects.filter(db_models.Q(holiday_date = today, is_deleted = False)):
            logger.warning("early check in Script can't be run on Holiday") 
            return
        
        q_filter = db_models.Q(employee__work_details__employee_status='Active',
                                date_of_checked_in=timezone_now().date(),
                                is_logged_out=False,
                                time_in__time__lte = db_models.F('employee__assignedattendancerules__attendance_rule__shift_in_time'),
        ...
        emps = attendance_models.EmployeeCheckInOutDetails.objects.filter(q_filter).annotate(
            ...
            ).values("rp_mail","rp_name",'employee__assignedattendancerules__attendance_rule__shift_in_time',
                    'employee__user__username','employee__official_email','time_in__time','employee__assignedattendancerules__attendance_rule',
                    'employee__company__company_name','employee__work_details__employee_number','work_rule_choices_new','employee__user__phone').distinct('id')
        ...
        It appears that you have Check-In early today. Kindly provide the reason to your manager {rep_name}.
                ...
                if user_phone and rep_name:
                    try:
                        whatsapp_data = {
                                ...
                                'subject': "Gentle Reminder: HRMS Check-In for Today",
                                "body_text1":"It appears that you have Check-In early today",
                        ...
                        logger.warning(f"Error while sending Whatsapp notificaton to {emp_name} in early checkin employee: {e}") 
    
    #early checkout 
if __name__ == "__main__":
    EmplEarlyCheckinData().main()
